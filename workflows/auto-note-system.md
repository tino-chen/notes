...
一套全自动的笔记系统：AI 在凌晨自动回顾当天的工作内容，识别值得沉淀的经验，撰写草稿、审核质量、构建发布、推送到 GitHub Pages。
...
每天 02:00 运行，完整 prompt：

```
执行协作回顾与笔记草稿撰写任务：

1. 读取 ~/.openclaw/workspace/memory/ 下最近的每日记忆文件（今天 + 前几天）
2. 回顾协作记录，识别值得沉淀为笔记的价值点，判断标准：
   - 完整的实践经验（从 0 到 1 的过程）
   - 非显而易见的踩坑和解决方案
   - 有启发性的架构思考和设计决策
   - 对其他 AI 协作者有参考意义的经验
3. 如果发现值得写的内容：
   a. 先读 ~/.openclaw/skills/note-taking/SKILL.md（全局规范）
   b. 如果分类是 workflows/，再读 ~/.openclaw/skills/workflow-note/SKILL.md（分类模板）
   c. 撰写笔记草稿，保存到 <笔记站路径>/_drafts/ 目录
   d. 文件名格式：YYYY-MM-DD-标题.md
4. 如果没有值得写的，安静结束

注意：
- 只写草稿，不要构建、不要提交、不要推送
- 草稿质量要高，因为后续会有独立的审核步骤
- 已写过的话题不要重复写，除非有重大更新
```
...
3. 如果有草稿，逐一审核每篇草稿，检查以下质量标准：
   ...
   信息泄露检查：
   - 是否暴露了不应公开的内部信息（IP、密码、token、私有路径等）
...
5. 审核完成、质量达标后：
   a. 将草稿从 _drafts/ 移动到对应分类目录（workflows/ 或 tools/）
   b. 更新 .vitepress/config.ts 的 sidebar 配置
   c. 更新分类目录下的 index.md
   d. 运行 npm run docs:build
   e. 运行 git add -A && git commit -m '笔记: [标题]' && git push
   f. 输出简要通知：发布了什么笔记及链接
```
