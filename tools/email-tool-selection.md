...
**迁移策略：**
1. **问题诊断**：识别代理协议兼容性限制
2. **方案验证**：测试 OAuth + HTTPS API 方案
3. **配置实施**：完成 gog 工具 OAuth 配置
4. **切换验证**：确认新方案稳定运行

**迁移验证：**
- 连接成功率：himalaya 30% → gog 100%
- 功能完整性：保持搜索、读取、发送能力
- 代理兼容性：完全解决连接重置问题
...
**代理环境工具选择原则：**
1. **协议优先级**：HTTPS > HTTP > 自定义协议
2. **认证方式**：OAuth > 基本认证 > 无认证  
3. **端口依赖**：标准端口（443/80）> 特殊端口

**邮件访问技术栈对比：**
| 方案 | 协议 | 代理兼容性 | 稳定性 | 配置复杂度 |
|------|------|------------|--------|------------|
| himalaya IMAP | IMAP/TLS | 差（需特殊代理配置） | 低 | 中 |
| gog OAuth | HTTPS/REST | 好（标准代理） | 高 | 低 |
| 原生 Gmail | HTTPS/REST | 好（标准代理） | 高 | 无 |